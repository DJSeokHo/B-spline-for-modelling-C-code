...
{
public:
	void Set_Data_Bsurf(int num_u, int ord_u, int bp_u, int num_v, int ord_v, int bp_v)
	{
		Num_u = num_u;
		ORD_u = ord_u;
		BP_u = bp_u;
		VER_u = BP_u;
		BP_ARR_u = bp_u + ord_u + 1;
		VER_ARR_u = VER_u + ord_u + 1;

		Num_v = num_v;
		ORD_v = ord_v;
		BP_v = bp_v;
		VER_v = BP_v;
		BP_ARR_v = bp_v + ord_v + 1;
		VER_ARR_v = VER_v + ord_v + 1;
	}
	
	void Cal_Data_Bsurf()
	{
		ksi = new double*[VER_u];
		for(int i=0;i<VER_u;i++)
		{
			ksi[i] = new double[VER_v];
		}

		eta = new double*[VER_u];
		for(i=0;i<VER_u;i++)
		{
			eta[i] = new double[VER_v];
		}

		zeta = new double*[VER_u];
		for(i=0;i<VER_u;i++)
		{
			zeta[i] = new double[VER_v];
		}
	}
	
	double **ksi, **eta, **zeta;
	int Num_u, Num_v, ORD_u, ORD_v, BP_u, BP_v, VER_u, VER_v, BP_ARR_u, BP_ARR_v, VER_ARR_u, VER_ARR_v, *pivot_u, *pivot_v;
};
