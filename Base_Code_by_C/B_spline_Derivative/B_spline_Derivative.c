...
double cacu_Nki_dev(int tk, double u, double ui, double Nk_i_p_1, double ui_k, double Nk_i_1_p_1, double ui_k_1, double ui_1);

main()
{
	FILE *test=fopen("test.dat","wt");
	FILE *CP=fopen("CP.dat","wt");
	int i, j, k, cacl_KV, spn[Num], n, cout, cot, dev;
	double x, y, u[Num], U[VER_ARR], N[ORD+1][VER], N_sig[Num][VER];	
	double ksi[VER], eta[VER], ksi_dev_1[VER-1], eta_dev_1[VER-1], ksi_dev_2[VER-2], eta_dev_2[VER-2];
	double N_dev_1[ORD+1][VER], N_sig_dev_1[Num][VER];
	double N_dev_2[ORD+1][VER], N_sig_dev_2[Num][VER];
...
//	printf("%d\n", cacl_KV);	//......check result
	...
	for(i=0;i<VER-1;i++)
	{
		...
		ksi_dev_1[i] = ((ORD)/(U[i+ORD+1]-U[i+1])) * (ksi[i+1]-ksi[i]);
		eta_dev_1[i] = ((ORD)/(U[i+ORD+1]-U[i+1])) * (eta[i+1]-eta[i]);
		
	//	fprintf(CP, "%lf %lf\n", ksi_dev_1[i], eta_dev_1[i]);
	}

	for(i=0;i<VER-2;i++)
	{
		ksi_dev_2[i] = ((ORD)/(U[i+ORD+1]-U[i+1])) * (ksi_dev_1[i+1]-ksi_dev_1[i]);
		eta_dev_2[i] = ((ORD)/(U[i+ORD+1]-U[i+1])) * (eta_dev_1[i+1]-eta_dev_1[i]);
		fprintf(CP, "%lf %lf\n", ksi_dev_2[i], eta_dev_2[i]);
	}
...
//		printf("%d\n", spn[i]);	//......check result
			...
			for(k=0;k<=ORD;k++)
			{
				...
				{	
					...
					else
					{
						...
						N_dev_1[k][i] = cacu_Nki_dev(k, u[n], U[i], N[k-1][i], U[i+k], N[k-1][i+1], U[i+k+1], U[i+1]);
						N_dev_2[k][i] = cacu_Nki_dev(k, u[n], U[i], N_dev_1[k-1][i], U[i+k], N_dev_1[k-1][i+1], U[i+k+1], U[i+1]);
					}
				...
				//	fprintf(test, "%lf\n", N_dev_2[ORD][cout]);
					...
					N_sig_dev_1[n][cout] = N[ORD][cout];
					N_sig_dev_2[n][cout] = N[ORD][cout];
				}
			}
		}
	}
	...
	//		y += eta[cot] * N_sig[n][cot];
		...
		for(cot=0;cot<VER-1;cot++)
		{
	//		x += ksi_dev_1[cot] * N_sig_dev_1[n][cot];
	//		y += eta_dev_1[cot] * N_sig_dev_1[n][cot];
		}
		for(cot=0;cot<VER-2;cot++)
		{
			x += ksi_dev_2[cot] * N_sig_dev_2[n][cot];
			y += eta_dev_2[cot] * N_sig_dev_2[n][cot];
		}
...
double cacu_Nki_dev(int tk, double u, double ui, double Nk_1i, double ui_k, double Nk_1i_1, double ui_k_1, double ui_1)
{
	double resl, temp_u, temp_ui, temp_Nk_1i, temp_ui_k, temp_Nk_1i_1, temp_ui_k_1, temp_ui_1, temp_left, temp_right;
	int temp_tk;
	temp_tk = tk; temp_u=u; temp_ui=ui; temp_Nk_1i=Nk_1i; temp_ui_k=ui_k; temp_Nk_1i_1=Nk_1i_1; temp_ui_k_1=ui_k_1; temp_ui_1=ui_1;
